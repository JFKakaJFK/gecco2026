...
Overall, the scope of this project is to be a good starting point for internal and external users. For that, it should be reasonably easy to use and feature-complete enough to encompass a wide range of use cases, from algorithm research and benchmarking to, e.g., survival analysis. The motivation for having a single joint library is that, with a bit of planning and collaboration, we should be able to get our work done more easily while maintaining a nice library. Ideally, the library shortens the time until running the first experiment.
...
- [Debugging and Performance Optimization](TODO how to run py/c++/mixed debuggers, bisection, reducing issues to minimum viable reproductions; Amdahl's law, profiling with samply, memory profiling with e.g. massif, timing with hyperfine/timeit, C++ data structure footguns, do less work ~> approximate work ~> do it faster, parallelism (OpenMP/Taskflow/TBB), GPU acceleration (CUDA/Metal/OpenGL), LAPACK/BLAS/BLIS)
