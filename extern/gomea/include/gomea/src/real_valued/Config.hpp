...
class Config {
 public:
  ...
  /*-=-=-=-=-=-=-=-=-=-=-=- Options -=-=-=-=-=-=-=-=-=-=-=-=-*/
  int problem_index = -1;
  bool use_guidelines = false, use_vtr = false,
      ...
      write_generational_population = false, selection_during_gom = false, update_elitist_during_gom = false,
       verbose = false;
  int number_of_variables = -1,           /* The number of variables in the optimization problem. */
      ...
      maximum_number_of_generations = -1, /* The maximum number of generations before any population in the multi-start
      ...
      maximum_no_improvement_stretch = 100; /* The maximum number of subsequent generations without an improvement while
  ...
  double maximum_number_of_evaluations = -1, /* The maximum number of evaluations. */
      ...
      st_dev_ratio_threshold =
          1.0, /* The maximum ratio of the distance of the average improvement to the mean compared to the distance of
                  one standard deviation before triggering AVS (SDR mechanism). */
