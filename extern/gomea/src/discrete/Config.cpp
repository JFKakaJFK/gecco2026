...
bool Config::parseCommandLine(int argc, char** argv) {
  ...
  options.add_options()("h,help", "Prints out usage information")("P,problems", "Prints out all installed problems")(
      ...
      ("VTRfile", "Path to VTR file.", cxxopts::value<std::string>())(
          "O,folder", "Folder where output is saved to. Default: \"output_discrete_gomea\"",
          cxxopts::value<std::string>())("F,max_number_of_evaluations",
                                         "Maximum number of function evaluations. Default: inf", cxxopts::value<int>())(
          ...
          "max_number_of_generations", "Maximum number of generations per population. Default: inf",
          cxxopts::value<int>())("IMS_subgeneration_factor",
                                 "Factor in which larger populations are started in IMS. Default: 4",
                                 cxxopts::value<int>())
      // Linkage config
      ("FOS_index", "Index of the FOS to be used.", cxxopts::value<int>())(
          ...
          "filtered_lt", "Enable for filtered linkage tree.")("static_lt", "Enable for static linkage tree.")(
  ...
  switch (FOSIndex) {
    ...
    case linkage::linkage_model_type::CONDITIONAL:
      linkage_config = new linkage_config_t(cond_max_clique_size, cond_include_cliques_as_fos_elements,
                                            cond_include_full_fos_element);
      break;
...
void Config::printOverview() {
  ...
  std::cout << "# similarity measure : " << (linkage_config->lt_similarity_measure ? "normalized MI" : "MI")
            << std::endl;
  ...
  std::cout << "# VTR                          = "
            << ((fitness->vtr < 1e+308) ? std::to_string(fitness->vtr) : "not set") << std::endl;
