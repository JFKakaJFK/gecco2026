...
def example_solution():
    ...
    fig.savefig(
        "template_example_2.pdf", dpi=600, transparent=True, bbox_inches="tight"
    )
...
def example_peter_proximity():
    ...
    node_proximity = ctx.normalized_node_proximity()
    vig_proximity = ctx.normalized_wVIG()
    ...
    fig, axes = plt.subplot_mosaic(
        ...
        """,
        figsize=(7, 3.5),
        gridspec_kw=dict(width_ratios=[1, 0.75], wspace=0.25),
    )
    ...
    # peter_proximity[:, :] = 0
    peter_proximity = np.where(peter_proximity > 1, 1, 0)
    peter_proximity[[2, 5, 6], :] = 0
    # peter_proximity = np.where(peter_proximity > 1, 1, 0)
    ...
    fig.savefig(
        "node_proximity_example_both_2.pdf",
        dpi=600,
        transparent=True,
        bbox_inches="tight",
    )
...
def linkage_example():
    ...
    fig, axes = plt.subplots(
        ...
        gridspec_kw=dict(
            hspace=0.4, height_ratios=[1, 0.75, 1], width_ratios=[0.75, 1, 1]
        ),
    )
    ...
    draw_measure(
        ...
        mask=np.eye(MI.shape[0], dtype=np.bool_),
    )
    draw_measure(
        H,
        None,
    ...
    draw_measure(H_masked, None, "$H_{masked}$", axes[2, 1], cbar=False)
    draw_measure(
        MI_masked,
        None,
        "$MI_{masked}$",
    ...
    draw_lt(
        Z, n, axes[2, 0], include_full_fos=True, label_kwargs=dict(font_size="small")
    )
    ...
    fig.savefig("linkage_example_2.pdf", dpi=600, transparent=True, bbox_inches="tight")


def linkage_example2():
    solutions, active = (
        [
            [r"$\sin$", "$x_0$", "$x_1$"],
            ["$x_0$", "$x_1$", "$x_0$"],
    ...
    #     cbar=False,  # , annot_kws=dict(fontsize="large")
    # )
    draw_measure(H_masked, None, "$H_{masked}$", axes[0, 1], cbar=False)
    draw_measure(
        MI_masked,
        None,
        "$MI_{masked}$",
    ...
    fig.savefig(
        "linkage_example_masked.pdf", dpi=600, transparent=True, bbox_inches="tight"
    )
