...
def run_one(
    ...
    with tempfile.TemporaryDirectory() as odir:
        ...
        alg = MixedGOMEA(
            ims_options=IMSOptions(initial_population_size=1024, max_num_populations=1),
        ...
        template = Template()
        template.add_output(TemplateNode.full_nary(2, 4))
        ctx = GPContext(
            num_inputs=int(X_train.shape[1]),
            expression_template=template,
            operators=[OpAdd(), OpSub(), OpMul(), OpDiv(), OpSin()],
        ...
        problem = SRProblem(
            ...
            init=HalfHalfInit(),
            constant_init_lower_bound=float(-np.nanmax(y_train)),
            constant_init_upper_bound=float(np.nanmax(y_train)),
        )

        logpath = pathlib.Path(odir) / "log.csv"
        ...
        Tracked.run(
            ...
            alg,
            budget,
            TrackingOptions(logpath=str(logpath)),
        )
        t_end = time.time()
        ...
        expr: str = str(df["expressions"].iloc[-1]).split(",")[0].replace("LS", "")
