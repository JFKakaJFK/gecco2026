    ...
    void evaluate_partial(SolutionBase& solution, const SolutionBase& parent, const Subset& subset) override final {
        ...
        for (usize i = 0; i < num_objectives(); i++) {
            auto [ov, cv] = objectives[i]->evaluate_partial(
                ...
                parent.discrete_active(), parent.continuous_active(), pq.objectives(i), pq.constraint_value,
...
class BenchmarkInstance final : public InstanceBase {
   public:
    BenchmarkInstance(std::variant<std::vector<std::shared_ptr<ObjectiveBase>>,
        ...
        // TODO for problem specified bounds/targets: either merge with user-provided information or only use one
        // instead of silently overwriting user provided parameters...
        ...
        // approximate the target with some tolerance instead of perfect recovery -> only register a target condition if
        // a tolerance is provided
        auto fn_front = _objectives->pareto_front();
        if (fn_front.has_value() && target_tolerance.has_value()) {
            add_target([pareto_front = std::move(fn_front.value()),
                        tolerance = target_tolerance.value()](const ArchiveBase& archive) {
                return archive.approximately_covers(*pareto_front, tolerance);
            });
        }
    };
    ...
    void set_initial_bounds(std::variant<CType, Vec<CType>> continuous_init_lower_bound = CType(0.0),
        ...
        __goblin_runtime_assert(static_cast<usize>(_continuous_init_lower_bounds.size()) ==
                                _objectives->num_continuous());
        ...
        __goblin_runtime_assert(static_cast<usize>(_continuous_init_upper_bounds.size()) ==
                                _objectives->num_continuous());
    };
