...
class Tracked final : public WrappedInstance {
   public:
    ...
    bool adapt(Rng& rng) override final {
        if (inner.adapt(rng)) {
            ...
            // counted, but with the consequence that if you track evaluations more evaluations than allowed by the
            // budget will be performed to arguably more correctly track the algorithm performance...
    ...
    static std::tuple<std::shared_ptr<ArchiveBase>, TerminationStatus> run(
        ...
        try {
            ...
            // TODO why is this necessary? (alg should not be able to hit the target without the exception being
            // thrown...)
   ...
   private:
    ...
    void wrap_eval(E eval, SolutionSetBase& solutions, const std::span<const usize>& indices) {
        ...
        // TODO if evaluations == 0 optionally add all solutions to the archive, not just indices => correct tracking
        // for warm start scenarios where some solutions already have a fitness

        // check if the budget was exhausted while the algorithm was running
        auto elapsed =
            config.consider_evaluation_time ? alg_timer.elapsed() + eval_timer.elapsed() : alg_timer.elapsed();
        generation = method.current_generation();
    ...
    bool should_report() {
        ...
        if (elapsed > time_elapsed_at_next_report) {
            report_needed = true;
            time_elapsed_at_next_report =
                std::min(config.time_factor > 1
                             ? std::chrono::duration_cast<std::chrono::nanoseconds>(elapsed * config.time_factor)
                             : std::chrono::duration_cast<std::chrono::nanoseconds>(
                                   elapsed + config.initial_time_until_next_report),
