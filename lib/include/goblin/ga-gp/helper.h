...
static int compute_block_size(int num_data_points) {
    ...
    // If there are multiple optimal solutions, the smallest block size is preferred
    // TODO test if this is beneficial in practice

    // Iterate over possible block sizes (multiples of 32)
    for (int b = 32; b <= MAX_THREADS_PER_BLOCK; b += 32) {
