    ...
    Mat<DType> sample(Rng& rng, const InstanceBase& problem, usize count) const override final {
        try {
            ...
            for (isize c = 0; c < dvals.cols(); c++) {
                ...
                double actual_p_terminal = p_terminal.value_or(1.0 - static_cast<double>(non_terminals.size()) /
        ...
        try {
            ...
            for (isize c = 0; c < dvals.cols(); c++) {
                ...
                if (is_leaf) {
                    ...
                    double actual_p_constant = p_constant.value_or(static_cast<double>(const_terminals.size()) /
                                                                   (static_cast<double>(const_terminals.size()) +
                                                                    static_cast<double>(non_const_terminals.size())));
                    if (const_terminals.empty()) {
                        actual_p_constant = 0.0;
                    }
                    ...
                    if (non_terminals.empty()) {
                        throw std::runtime_error(
                            "Variable domain must include non-terminal symbols for non-leaf nodes.");
                    }
        ...
        try {
            ...
            for (isize i = 0; i < dvals.rows(); i++) {
                for (usize root_idx = 0; root_idx < num_roots; root_idx++) {
                    usize root = root_idx < ctx.output_roots.size()
                                     ? ctx.output_roots[root_idx]
                                     : ctx.subtree_roots[root_idx - ctx.output_roots.size()];
                    sample_tree(rng, problem, ctx, non_terminals, const_terminals, non_const_terminals, dvals.row(i),
                                root);
                }
            }
            return dvals;
...
class RecursiveCompleteInit2 final : public DiscreteInitBase {
        ...
        try {
            ...
            for (usize root_idx = 0; root_idx < num_roots; root_idx++) {
                ...
                while (!queue.empty()) {
                    ...
                    dvals(inactive_indices, current) =
                        sample_nodes(rng, problem, non_terminals, const_terminals, non_const_terminals,
                                     inactive_indices.size(), current);
   ...
   private:
    Vec<DType> sample_nodes(Rng& rng,
        ...
        double actual_p_terminal = p_terminal.value_or(1.0 - static_cast<double>(non_terminals[idx].size()) /
                                                                 static_cast<double>(domain_size));
        double actual_p_constant = p_constant.value_or(
            static_cast<double>(const_terminals[idx].size()) /
            (static_cast<double>(const_terminals[idx].size()) + static_cast<double>(non_const_terminals[idx].size())));
