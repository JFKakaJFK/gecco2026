...
class PyGPInstance : public GPInstanceBase {
   ...
   public:
    ...
    virtual std::tuple<bool, bool> inherit_discrete(SolutionBase& offspring,
        ...
        // the pool size is not tied to the number of discrete variables, so the full pool instead of the paired values
        // is inherited...
        ...
        if (inherit_continuous && !inherit_by_index) {
            // note: arguably just inheriting all continuous variables even if the inherited discrete values might not
            // even be constants is not the best idea - but earlier experiments on another codebase suggested that more
            // appropriate/interpolating continuous mixing doesn't really work and here it also is more for completeness
