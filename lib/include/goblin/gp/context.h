...
class GPContext {
   public:
    ...
    template <typename S>
    std::optional<std::vector<std::vector<usize>>> nodes_post_order(S& solution,
                                                                    bool discount_size,
                                                                    usize& size) const {
        ...
        // in the modular GP-GOMEA paper (https://arxiv.org/pdf/2505.01262v1) there is this concept of "discounted" size
        // to not punish re-using subfunctions by only counting the subfunction nodes once. unless subfunctions are
        // enabled, the discounting has no effect
        ...
        // for each we need to visit, we need the node index, the call stack idx and whether the node already was
        // visited (for functions the first time is in-order, and the second time is post-order)
        ...
        size = 0;  // initially the size is 0 (size in GP is somewhat arbitary - even without subftrees/args which are
                   // not counted, simplification typically also has an effect and it's not necessarily a good proxy for
        ...
        for (usize n : output_roots) {
            ...
            // nodes are visited up to twice - once in-order, and once post-order after the children have been taken
            // care of
            while (!node_stack.empty()) {
                ...
                // we only need to look at the node if this is the first time we see it - in the post-order visit all we
                // have to do is add it to the tree
                if (!is_post_order) {
                    ...
                    // - in no case are references counted, i.e. the visit count for arg/fn nodes needs to be reset to 0
                    // later on track the size (repeat visits don't count in the "discounted" setting)
                    if (discount_size) {
                        visited(idx) = 1;
                    }
                    ...
                    if (value_kind[value] == ValueKind::Arg) {
                        ...
                        while (true) {
                            ...
                            if (value_kind[child_value] != ValueKind::Arg) {
                                ...
                                assert(root[idx] <= root[child] &&
                                       "Caller and child must originate from a hierarchically higher tree to ensure "
                                       "there are no cycles.");
                                node_stack.pop_back();
                        ...
                        std::get<2>(node_stack[node_stack_idx]) =
                            true;  // this is a reference type, but we need the post-order visit to keep the call stack
                                   // in sync
                        node_stack.emplace_back(
                            subtree_roots[v_idx],
                            call_stack.size() -
                                1,  // a call always needs to use the top of the stack, no
                        ...
                        // for anything that is not a leaf, we need to resolve the arguments, so another branch is
                        // needed if non-terminal kinds are added
                    ...
                    if (value_kind[value] == ValueKind::Subtree) {
                        // in the previous in-order visit, this node was pushed on the call stack so it has to be
                        // removed now
                        call_stack.pop_back();
        ...
        for (usize i = 0; i < trees.size(); i++) {
            ...
            for (usize j = 0; j < tree.size(); j++) {
                ...
                if (value_kind[value] == ValueKind::Input) {
                    ...
                    // the arguments are in the correct order on the stack, so we just need to get the last arity
                    // indices on the arg_stack
                    std::span<const std::string> args{arg_stack.end() - arity, arg_stack.end()};
                    ...
                    std::unreachable();  // if this triggers, either not all reference types have been removed or a
                                         // non-reference value kind has been added...
                }
    ...
    std::optional<Arr2D<Scalar>> compute_outputs(Arr2D<Scalar>& eval_buffer,
        ...
        // retrieved from the stack and at the end, the single stack entry is the result. Since arguments might be
        // consist of nested operations, the buffer indices corresponding to the actual results are needed somewhere.
        ...
        for (usize i = 0; i < trees.size(); i++) {
            ...
            for (usize j = 0; j < tree.size(); j++) {
                ...
                if (value_kind[value] == ValueKind::Input) {
                    ...
                    if (use_apply_buf) {
                        ...
                        operators[v_idx]->apply(eval_buffer.col(j),
                                                eval_buffer(Eigen::placeholders::all, child_indices));
                    }
            ...
            // at the end the stack only contains the tree output, and that is at the buffer position for the last tree
            // node
        ...
        for (usize i = 0; i < num_discrete; i++) {
            for (usize j = 0; j <= i; j++) {
                if (root[i] == root[j]) {
                    proximity(i, j) = 0.0;
                    usize ni = i, nj = j;
                    // while the lowest common ancestor was not found, replace the deeper node with its parent until the
                    // paths meet at the closest common ancestor
                    while (ni != nj) {
                        if (depth[ni] > depth[nj]) {
                            assert(parent(ni).has_value() &&
                                   "Since depth > 0, either the depth or parent lookup tables are wrong.");
                            ni = parent(ni).value();
                        } else {
                            assert(depth[nj] > 0 &&
                                   "Both are not the same, so at least one must have a non-zero depth since i and j "
                                   "are in the same "
                                   "tree");
                            assert(parent(nj).has_value() &&
                                   "Since depth > 0, either the depth or parent lookup tables are wrong.");
                            nj = parent(nj).value();
                        }
                        proximity(i, j) += 1.0;
    ...
    // The variables are marked active/inactive only if the solution is not const.
    template <typename S>
    bool to_gpu_representation(S& solution,
        ...
        // for each output, walk the tree in post-order
        size = 0;  // initially the size is 0 (size in GP is somewhat arbitrary - even without subtrees/args which are
                   // not counted, simplification typically also has an effect and it's not necessarily a good proxy for
                   // "interpretability" in the first place)
