   ...
   public:
        ...
        if (custom_similarity.has_value()) {
            if (static_cast<usize>(custom_similarity.value().rows()) != subset.size() ||
                ...
                throw std::runtime_error(
                    "Custom similarity matrix has invalid dimensions, expected a square matrix corresponding to the "
                    "subset "
                    "size.");
            }
        }
    ...
    Mat<CType> compute_similarity(
        ...
        if (custom_similarity.has_value() && !eta_custom_similarity.has_value()) {
            ...
            similarity = estimate_entropy(problem, solutions, indices, subset.discrete, intron_strategy,
            ...
            if (!covariance.has_value()) {
                ...
                throw std::runtime_error(
                    "Covariance matrix has invalid dimensions, expected a square matrix corresponding to the subset "
                    "size.");
            }
    ...
    FOS learn_lt_original(Rng& rng, Mat<CType> similarity, bool filter_root_default) const {
        ...
        while (!done) {
            ...
            if (r1 < mpm.size()) {
                ...
                double mul0 =
                    static_cast<double>(mpm[r0].size()) / static_cast<double>(mpm[r0].size() + mpm[r1].size());
                double mul1 =
                    static_cast<double>(mpm[r1].size()) / static_cast<double>(mpm[r0].size() + mpm[r1].size());
