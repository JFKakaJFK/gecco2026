...
class SolutionBase {
   public:
    ...
    // Instead of vector, should this be an ExtensionProxy that behaves like a vector/iterator but does not allocate
    // full copies?? (size, begin, end, proxy to underlying collection)
...
class SoASet : public SolutionSetBase {
   public:
    ...
    // I'm not sure if that is still the case, but the same idea holds if the whole SoASet is moved (e.g. when you have
    // a vector of populations that gets resized and all populations move so that the arena pointer in the solution is
    // out of date)
