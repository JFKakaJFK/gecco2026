...
class InstanceBase {
   ...
   public:
    ...
    /// The offspring inherits a subset of the decision variables from the donor, returning true if there was a change
    /// to the active variables and an evaluation is needed, possibly with problem specific modifications.
    ///
    ...
    InstanceBase& add_target(std::function<bool(const ArchiveBase&)> target_reached_check) {
        if (_target_reached) {
            ...
            _target_reached = [a = std::move(existing), b = std::move(target_reached_check)](
                                  const ArchiveBase& archive) { return a(archive) || b(archive); };
        } else {
    ...
    InstanceBase& add_target(std::variant<Vec<CType>, std::vector<CType>> target_objectives) {
        ...
        if (auto p = dynamic_cast<const MOQuality*>(vtr.get()); p == nullptr) {
            throw std::runtime_error(
                "A vector of objective values is only a valid target value in combination with `MOQuality` or a "
                "subclass "
                "as quality.");
        }
    ...
    InstanceBase& add_target_front_size(usize target_front_size) {
        return add_target(
            [target_front_size](const ArchiveBase& archive) { return archive.size() >= target_front_size; });
    }
    InstanceBase& add_target_front(Mat<CType> target_objectives) {
        auto vtr = archive_fitness().worst();
        if (auto p = dynamic_cast<const MOQuality*>(vtr.get()); p == nullptr) {
            throw std::runtime_error(
                "A matrix of objective values is only a valid target value in combination with `MOQuality` or a "
                "subclass "
                "as quality.");
        }
        ...
        for (isize i = 0; i < target_objectives.rows(); i++) {
            ...
            target_front.update(
                Solution(vtr->clone(), Vec<DType>::Zero(num_discrete()), Vec<CType>::Zero(num_continuous())));
        }
    ...
    InstanceBase& add_target(CType target_fitness) {
        if (num_objectives() != 1) {
            throw std::runtime_error(
                "A single objective value is only a valid target value for single-objective instances.");
        }
        auto vtr = archive_fitness().worst();
        if (auto p = dynamic_cast<const MOQuality*>(vtr.get()); p == nullptr) {
            throw std::runtime_error(
                "A single objective value is only a valid target value in combination with `MOQuality` or a subclass "
                "as "
                "quality.");
        }
