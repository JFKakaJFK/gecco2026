...
TEST_CASE("goblin::gp::sr") {
  ...
  GPContext ctx(
      /* num_inputs = */ X.cols(),
      tmplate, operators, /* num_parameters = */ 0, /* constant_representation = */ "ercs", /* constant_pool_size = */ 0, /* enable_subfunctions = */ true);

  auto f = MOFitness(1);
  Solution s(f.worst(), Vec<DType>::Zero(ctx.num_discrete),
             Vec<CType>::Zero(ctx.num_continuous));

  // 0 -> x0
  // 1 -> x1
  // 2 -> constant
  // 3 -> arg[0] / fn[0] / op[0]
  s.discrete_values()(ctx.subtree_roots[0]) = ctx.value2domain(ctx.subtree_roots[0], ctx.op_idx2value[0]).value_or(1000); // -
  REQUIRE(ctx.subtree_roots[0] == 0);
  REQUIRE(s.discrete_values()(0) == 5);
  s.discrete_values()(ctx.children[ctx.subtree_roots[0]][0]) = 3; // Arg[0]
  REQUIRE(ctx.children[ctx.subtree_roots[0]][0] == 1);
  REQUIRE(s.discrete_values()(1) == 3);
  s.discrete_values()(ctx.children[ctx.subtree_roots[0]][1]) = 4; // Arg[1]
  REQUIRE(ctx.children[ctx.subtree_roots[0]][1] == 2);
  REQUIRE(s.discrete_values()(2) == 4);

  s.discrete_values()(ctx.subtree_roots[1]) = 5; // Fn[0]
  REQUIRE(ctx.subtree_roots[1] == 3);
  REQUIRE(s.discrete_values()(3) == 5);
  s.discrete_values()(ctx.children[ctx.subtree_roots[1]][0]) = 4; // Arg[1]
  REQUIRE(ctx.children[ctx.subtree_roots[1]][0] == 4);
  REQUIRE(s.discrete_values()(4) == 4);
  s.discrete_values()(ctx.children[ctx.subtree_roots[1]][1]) = 3; // Arg[0]
  REQUIRE(ctx.children[ctx.subtree_roots[1]][1] == 5);
  REQUIRE(s.discrete_values()(5) == 3);

  s.discrete_values()(ctx.output_roots[0]) = 4; // Fn[1]
  REQUIRE(ctx.output_roots[0] == 6);
  REQUIRE(s.discrete_values()(6) == 4);
  s.discrete_values()(ctx.children[ctx.output_roots[0]][0]) = 0; // x0
  REQUIRE(ctx.children[ctx.output_roots[0]][0] == 7);
  REQUIRE(s.discrete_values()(7) == 0);
  s.discrete_values()(ctx.children[ctx.output_roots[0]][1]) = 1; // x1
  REQUIRE(ctx.children[ctx.output_roots[0]][1] == 8);
  REQUIRE(s.discrete_values()(8) == 1);
  ...
  Array<CType> params;
  Arr2D<CType> out = ctx.compute_outputs<CType>(s, X, params);
