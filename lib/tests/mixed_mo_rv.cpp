...
TEST_CASE("goblin::methods::mo_rv_gomea") {
    ...
    auto gomea = MixedGOMEA(PopulationOptions{.forced_improvements = false}, RvOptions(),
                            IMSOptions{.initial_population_size = 40,
    ...
    // Idea: use the D_{PF<-S} measure to ensure that for each point on the true PF (FRONT_SIZE samples) there is a
    // member in the approximation front within a distance of TOL
