...
struct NoisyResult : public QualityBase {
    ...
    std::unique_ptr<QualityBase> clone() const override final {
        return std::make_unique<NoisyResult>(*this);
    }
};
...
Ordering cmp(const QualityBase& lhs, const QualityBase& rhs, std::optional<usize> objective) const override final {
  ...
  CType distance(const QualityBase& lhs, const QualityBase& rhs, std::optional<usize> objective) const override final {
      ...
      return std::abs(
          (ql.samples.sum() / static_cast<CType>(ql.samples.size()))
...
class NoisySphere : public InstanceBase {
 public:
 usize num_discrete() const override final {
     return 0;
  }
 ...
 usize num_continuous() const override final {
     return D;
  }
 ...
 const FitnessBase& fitness() const override final {
return fitness_;
  }

 const ArchiveFitnessBase& archive_fitness() const override final {
return fitness_;
  }
