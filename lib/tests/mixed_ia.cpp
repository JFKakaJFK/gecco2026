...
TEST_CASE("goblin::methods::mixed_ia") {
  ...
  auto gomea = MixedGOMEA(
      options, RvOptions{.enabled=false}, IMSOptions{.initial_population_size = 2, .max_num_populations = 25, .restart_stale_populations = false}, std::make_shared<LinkageTreeFOS>(
          /* metric = */ "mi",
      ...
      , std::make_shared<FullFOS>(),
                          std::make_shared<AMaLGaMSamplingModel>());
