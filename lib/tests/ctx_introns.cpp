    ...
    for (usize es = 0; es <= 1; es++) {
        for (usize ls = 0; ls <= 1; ls++) {
            for (auto constant_representation : constant_reprs) {
                ...
                SRProblem srp(ctx, X, Y, std::nullopt, std::nullopt, objectives,
                              /* objectives_to_optimize = */ std::nullopt, linear_scaling);
                ...
                for (usize i = 0; i < N; i++) {
                    ...
                    //   srp.fitness().format(solutions[i].quality()),
                    //   format_as(srp.fitness().cmp(solutions[i].quality(), worst, std::nullopt)));
                    // }
                }
