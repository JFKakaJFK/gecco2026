...
TEST_CASE("goblin::gp::ctx_introns") {
  ...
  auto tree = TemplateNode::full_nary(/* branching_factor = */ 3, /* depth */ 2);
  Template tmplate;
  ...
  std::vector<std::shared_ptr<OperatorBase>> operators = {
      std::make_shared<OpAdd>(),    std::make_shared<OpSub>(),  std::make_shared<OpMul>(), std::make_shared<OpDiv>(),
      std::make_shared<OpSin>(),    std::make_shared<OpCos>(),  std::make_shared<OpExp>(), std::make_shared<OpLog>(),
      std::make_shared<OpSquare>(), std::make_shared<OpSqrt>(), std::make_shared<OpPow>(), std::make_shared<OpAbs>(),
      std::make_shared<OpMin>(),    std::make_shared<OpMax>()};
  ...
  for (usize es = 0; es <= 1; es++) {
    for (usize ls = 0; ls <= 1; ls++) {
      ...
      for (usize i = 0; i < N; i++) {
        ...
        if (srp.fitness().cmp(solutions[i].quality(), worst, std::nullopt) != Ordering::Better) {
          ...
          REQUIRE_MESSAGE(
              dist == doctest::Approx(0.0), srp.format_solution(before), " != ", srp.format_solution(solutions[i]), "(",
              srp.InstanceBase::format_solution(before), " != ", srp.InstanceBase::format_solution(solutions[i]), " / ",
              srp.fitness().format(before.quality()), " != ", srp.fitness().format(solutions[i].quality()), ")");
        }
      }
    }
  }
