...
TEST_CASE("goblin::gp::ctx_introns") {
    // idea: generate and evaluate random solutions, then randomly mess with the introns and evaluate - if there are introns that change the value, that's a problem...
 const usize N = 1000;
 ...
 std::iota(indices.begin(), indices.end(), 0);
  ...
  for(usize es = 0; es <= 1; es++){
    ...
    GPContext ctx(
        /* num_inputs = */ X.cols(), tmplate, operators, /* num_parameters = */ 0, /* constant_representation = */ "pool",
        /* constant_pool_size = */ 10, enable_subfunctions);

    SRProblem srp(ctx, X, Y, std::nullopt, std::nullopt, objectives, /* objectives_to_optimize = */ std::nullopt, linear_scaling);
    ...
    for(usize i = 0; i < N; i++){
        ...
        REQUIRE_MESSAGE(
            dist == doctest::Approx(0.0),
            srp.format_solution(before), " != ",
            srp.format_solution(solutions[i]), "(",
            srp.InstanceBase::format_solution(before),
            " != ",
