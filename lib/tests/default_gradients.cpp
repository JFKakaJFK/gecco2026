...
TEST_CASE("goblin::lib::instance_default_gradient") {
    ...
    SUBCASE("gradients()") {
        ...
        for (usize i = 0; i < 100; i++) {
            ...
            if ((i + 1) % 10 == 0) {
                sphere.evaluate(rng, solutions, indices);
                REQUIRE_MESSAGE(sphere.fitness().cmp(solutions[0].quality(), *prev, std::nullopt) == Ordering::Better,
                                i, ", P:", sphere.fitness().format(*prev),
                                ", C:", sphere.fitness().format(solutions[0].quality()),
