...
Mat<CType> estimate_entropy_impl(const InstanceBase& problem,
    ...
    if (problem.num_continuous() > 0) {
        if (merge_continuous) {
            ...
            while (i < indices.size() && continuous_bin_values_sorted.size() < num_continuous_bins.value()) {
                ...
                // already seen before
                if (idx < continuous_bin_values_sorted.size() &&
    ...
    // loop over all solutions and indices and prepare the value (is read multiple times, and lots of expensive
    // branching inside of the loop...)
...
Mat<CType> estimate_entropy(const InstanceBase& problem,
    ...
    if (intron_strategy == "any_active") {
        ...
        return estimate_entropy_impl<DiscreteIntronStrategy::None>(problem, solutions, indices, subset,
                                                                   merge_continuous, num_continuous_bins);
