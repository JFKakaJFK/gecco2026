...
short betterFitness(double objective_value_x,
...
void run(std::optional<goblin::usize> base_population_size = std::nullopt);
void runOnce(void);
...
std::mutex global_instance_mutex; /* We can only use the global variables once
                                     at the same time... */
std::function<void(double* parameters, double* objective_value, double* constraint_value)>
    global_evaluate;                                   /* Dynamic evaluation function */
std::vector<std::tuple<double, double>> global_bounds; /* Dynamic bounds */

short write_generational_statistics,    /* Whether to compute and write statistics
                                           every generation (0 = no). */
    write_generational_solutions,       /* Whether to write the population every
                                           generation (0 = no). */
...
int number_of_parameters,               /* The number of parameters to be optimized. */
    ...
    number_of_evaluations,              /* The current number of times a function evaluation
                                           was performed. */
    number_of_generations,              /* The current generation count. */
    number_of_populations,              /* The number of parallel populations that initially
    ...
    *samples_drawn_from_normal,         /* The number of samples drawn from the i-th
                                           normal in the last generation. */
    *out_of_bounds_draws,               /* The number of draws that resulted in an
                                           out-of-bounds sample. */
    *no_improvement_stretch,            /* The number of subsequent generations without an
                                           improvement while the distribution multiplier is
                                           <= 1.0, for each population separately. */
    maximum_no_improvement_stretch,     /* The maximum number of subsequent
                                           generations without an improvement while
                                           the distribution multiplier is <= 1.0. */
    maximum_number_of_populations;      /* The maximum number of parallel populations
                                           that initially partition the search space.
                                         */
double tau,                             /* The selection truncation percentile (in [1/population_size,1]). */
    alpha_AMS,                          /* The percentile of offspring to apply AMS (anticipated mean
                                           shift) to. */
    ...
    **constraint_values,                /* Sum of all constraint violations for population
                                           members. */
    ...
    **constraint_values_selections,     /* Sum of all constraint violations of
                                           selected solutions. */
    ...
    *upper_init_ranges,                 /* The initialization range upper bound */
    lower_user_range,                   /* The initial lower range-bound indicated by the user
                                           (same for all dimensions). */
    upper_user_range,                   /* The initial upper range-bound indicated by the user
                                           (same for all dimensions). */
    rotation_angle,                     /* The angle of rotation to be applied to the problem. */
    *distribution_multipliers,          /* Distribution multipliers (AVS mechanism), one
                                           for each population. */
    distribution_multiplier_increase,   /* The multiplicative distribution
                                           multiplier increase. */
    distribution_multiplier_decrease,   /* The multiplicative distribution
                                           multiplier decrease. */
    st_dev_ratio_threshold,             /* The maximum ratio of the distance of the average
                                           improvement to the mean compared to the distance
                                           of one standard deviation before triggering AVS
                                           (SDR mechanism). */
    vtr,                                /* The value-to-reach (function value of best solution that is
                                           feasible). */
    fitness_variance_tolerance,         /* The minimum fitness variance level that is
                                           allowed. */
    **mean_vectors,                     /* The mean vectors, one for each population. */
    **mean_vectors_previous,            /* The mean vectors of the previous generation, one
                                           for each population. */
    ***covariance_matrices,             /* The covariance matrices to be used for sampling,
                                           one for each population. */
    ***cholesky_factors_lower_triangle, /* The unique lower triangular matrix of
                                           the Cholesky factorization. */
    nextNextGaussian,                   /* Internally used variable for sampling the normal
                                           distribution. */
    ...
    *best_so_far_solution,              /* Best solution found so far over all starts. */
    best_so_far_objective_value,        /* Objective value of best solution found so
                                           far over all starts. */
    best_so_far_constraint_value;       /* Sum-of-constraints value of best solution
                                           found so far over all starts. */
