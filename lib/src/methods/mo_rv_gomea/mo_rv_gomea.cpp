...
void interpretCommandLine(int argc, char** argv) {
    ...
    if (use_guidelines) {
        ...
        if (maximum_number_of_populations == 1) {
            ...
            // base_population_size           = (int) (0.5*((double)
            // base_number_of_mixing_components)*(10.0*pow((double) number_of_parameters,0.5))); base_population_size =
            // (int) (0.5*base_number_of_mixing_components*(17.0
...
void printUsage(void) {
    ...
    printf(
        "     -4 for bounded fixed linkage tree learned from distance measure, -5 for fixed bounded linkage tree "
        "learned "
...
void initializeNewPopulationMemory(int population_index) {
    ...
    for (i = 0; i < selection_sizes[population_index]; i++) {
        ...
        objective_values_selection_previous[population_index][i] =
...
void initializeDistributionMultipliers(int population_index) {
    ...
    for (i = 0; i < number_of_mixing_components[population_index]; i++) {
        ...
        out_of_bounds_draws[population_index][i] =
            (int*)Malloc(linkage_model[population_index][i]->length * sizeof(int));
    }
}
...
void initializePopulationAndFitnessValues(int population_index) {
    ...
    for (i = 0; i < population_sizes[population_index]; i++) {
        installedProblemEvaluation(problem_index, populations[population_index][i], number_of_parameters, NULL, NULL,
                                   NULL, 0);
...
short checkVTRTerminationCondition(void) {
    ...
    if (use_vtr && haveDPFSMetric()) {
        ...
        metric_elitist_archive = computeDPFSMetric(default_front, default_front_size, elitist_archive,
                                                   elitist_archive_size, elitist_archive_indices_inactive);
...
void makeSelection(int population_index) {
    ...
    for (i = 0; i < selection_sizes[population_index]; i++)
        for (j = 0; j < number_of_objectives; j++)
            objective_values_selection_previous[population_index][i][j] =
                selection[population_index][i]->objective_values[j];
    ...
    for (i = 0; i < population_sizes[population_index]; i++) {
        if (pop_indices_selected[population_index][i] == -1) {
            ...
            for (k = 0; k < number_of_objectives; k++)
                selection[population_index][j]->objective_values[k] =
                    populations[population_index][i]->objective_values[k];
...
void estimateParameters(int population_index) {
    ...
    double **objective_values_selection_scaled, **objective_values_selection_previous_scaled, distance,
        distance_smallest, distance_largest, **objective_means_scaled_new, **objective_means_scaled_previous,
        *distances_to_cluster, **distance_cluster_i_now_to_cluster_j_previous,
        **distance_cluster_i_now_to_cluster_j_now, **distance_cluster_i_previous_to_cluster_j_previous, epsilon;
    ...
    while (epsilon > 1e-10) {
        ...
        for (i = 0; i < selection_sizes[population_index]; i++) {
            ...
            distance_smallest = -1;
            for (j = 0; j < number_of_mixing_components[population_index] - number_of_objectives; j++) {
                distance = distanceEuclidean(objective_values_selection_scaled[i],
                                             objective_means_scaled[population_index][j], number_of_objectives);
    ...
    for (i = 0; i < number_of_mixing_components[population_index] - number_of_objectives; i++) {
        for (j = 0; j < selection_sizes[population_index]; j++)
            distances_to_cluster[j] =
                distanceEuclidean(objective_values_selection_scaled[j], objective_means_scaled[population_index][i],
                                  number_of_objectives);
    ...
    if (number_of_generations[population_index] > 0) {
                ...
                for (k = 0; k < cluster_sizes[population_index]; k++) {
                    ...
                    for (q = 0; q < cluster_sizes[population_index]; q++) {
                        distance = distanceEuclidean(
                            objective_values_selection_scaled
                                [selection_indices_of_cluster_members_before_registration[i][k]],
                            objective_values_selection_previous_scaled
            ...
            for (j = 0; j < number_of_mixing_components[population_index]; j++) {
                ...
                if (i != j) {
                    for (k = 0; k < cluster_sizes[population_index]; k++) {
                        ...
                        for (q = 0; q < cluster_sizes[population_index]; q++) {
                            distance =
                                distanceEuclidean(objective_values_selection_scaled
                                                      [selection_indices_of_cluster_members_before_registration[i][k]],
                                                  objective_values_selection_scaled
                                                      [selection_indices_of_cluster_members_before_registration[j][q]],
                                                  number_of_objectives);
                            if ((distance_smallest < 0) || (distance < distance_smallest))
                                distance_smallest = distance;
                        }
        ...
        /* NEW: distance between clusters is the distance between the cluster means in scaled-objective space.
           ...
           objective_means_scaled[population_index][i], objective_means_scaled[population_index][j],
           number_of_objectives ); distance_cluster_i_now_to_cluster_j_now[j][i] =
           distance_cluster_i_now_to_cluster_j_now[i][j];
                }
              }
            }
        ...
        */
        ...
        while (number_of_clusters_left_to_register > 0) {
            ...
            sorted = mergeSort(distance_cluster_i_now_to_cluster_j_now[i_min],
            ...
            for (i = 0; i < number_of_cluster_permutations; i++) {
                ...
                if ((distance_smallest < 0) || (distance < distance_smallest)) {
                    distance_smallest = distance;
                    for (j = 0; j < number_of_clusters_to_register_by_permutation; j++)
                        nearest_neighbour_choice_best[j] =
            ...
            for (i = 0; i < number_of_clusters_to_register_by_permutation; i++) {
                ...
                if (r_nearest_neighbours_now[i] >=
                    number_of_mixing_components[population_index] - number_of_objectives) {
                    single_objective_clusters[population_index][nearest_neighbour_choice_best[i]] =
                        r_nearest_neighbours_now[i] -
                        (number_of_mixing_components[population_index] - number_of_objectives);
    ...
    for (i = 0; i < population_sizes[population_index]; i++) {
        ...
        index_smallest = -1;
        for (j = 0; j < number_of_mixing_components[population_index]; j++) {
            distance = distanceEuclidean(objective_values_scaled, objective_means_scaled[population_index][j],
                                         number_of_objectives);
            if ((distance_smallest < 0) || (distance < distance_smallest)) {
                index_smallest = j;
                distance_smallest = distance;
        ...
        for (j = 0; j < number_of_parameters; j++) {
            ...
            for (k = 0; k < cluster_sizes[population_index]; k++)
                mean_vectors[population_index][i][j] +=
                    selection[population_index][selection_indices_of_cluster_members[population_index][i][k]]
                        ->parameters[j];
            ...
            for (j = 0; j < linkage_model[population_index][cluster_index]->set_length[i]; j++) {
                ...
                for (k = j; k < linkage_model[population_index][cluster_index]->set_length[i]; k++) {
                    ...
                    for (m = 0; m < cluster_sizes[population_index]; m++)
                        cov += (selection[population_index]
                                         [selection_indices_of_cluster_members[population_index][cluster_index][m]]
                                             ->parameters[vara] -
                                mean_vectors[population_index][cluster_index][vara]) *
                               (selection[population_index]
                                         [selection_indices_of_cluster_members[population_index][cluster_index][m]]
                                             ->parameters[varb] -
...
void copyBestSolutionsToPopulation(int population_index, double** objective_values_selection_scaled) {
    ...
    for (i = 0; i < elitist_archive_size; i++) {
        ...
        for (j = 0; j < number_of_objectives; j++)
            objective_values_scaled[j] =
                elitist_archive[i]->objective_values[j] / objective_ranges[population_index][j];
        j_min = -1;
        distance_smallest = -1;
        for (j = 0; j < number_of_mixing_components[population_index]; j++) {
            distance = distanceEuclidean(objective_values_scaled, objective_means_scaled[population_index][j],
...
void estimateFullCovarianceMatrixML(int population_index, int cluster_index) {
    ...
    for (j = 0; j < number_of_parameters; j++) {
        for (q = j; q < number_of_parameters; q++) {
            cov = 0.0;
            for (k = 0; k < cluster_sizes[population_index]; k++)
                cov += (selection[population_index][selection_indices_of_cluster_members[population_index][i][k]]
                            ->parameters[j] -
                        mean_vectors[population_index][i][j]) *
                       (selection[population_index][selection_indices_of_cluster_members[population_index][i][k]]
                            ->parameters[q] -
...
void evaluateCompletePopulation(int population_index) {
    int i;
    for (i = 0; i < population_sizes[population_index]; i++) {
        installedProblemEvaluation(problem_index, populations[population_index][i], number_of_parameters, NULL, NULL, 0,
                                   0);
        if (global_terminate_immediately) {
            return;
        }
...
void applyForcedImprovements(int population_index, int individual_index, short* improved) {
    ...
    for (j = 0; j < elitist_archive_size; j++) {
        if (elitist_archive_indices_inactive[j])
            continue;
        for (k = 0; k < number_of_objectives; k++)
            objective_values_scaled[k] =
                elitist_archive[j]->objective_values[k] / objective_ranges[population_index][k];
    ...
    do {
        ...
        if (times_not_in_bounds >= 100) {
            ...
            if (use_univariate_FOS) {
                ...
                result[0] =
                    z[0] * sqrt(decomposed_covariance_matrices[population_index][cluster_index][FOS_index][0][0]) +
                    mean_vectors[population_index][cluster_index][indices[0]];
            } else {
                result = matrixVectorMultiplication(
                    decomposed_cholesky_factors_lower_triangle[population_index][cluster_index][FOS_index], z,
...
double getStDevRatioForOneClusterForFOSElement(int population_index,
    ...
    if (use_univariate_FOS) {
        result =
            fabs(x_min_mu[0] / sqrt(decomposed_covariance_matrices[population_index][cluster_index][FOS_index][0][0]));
    } else {
...
std::tuple<std::shared_ptr<ArchiveBase>, TerminationStatus> MoRvGOMEA::run(InstanceBase& problem,
    ...
    if (population_size.has_value()) {
        ...
        if (mo_rv_gomea_impl::maximum_number_of_populations > 1) {
            ...
            mo_rv_gomea_impl::base_population_size = initial_population_size.value_or(
                (int)((0.5 * mo_rv_gomea_impl::base_number_of_mixing_components) *
