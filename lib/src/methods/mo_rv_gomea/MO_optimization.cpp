    ...
    if (index == 10) {
        ...
        if (touched_parameters_indices != NULL && !black_box_evaluations) {
            ...
            number_of_evaluations +=
                static_cast<double>(touched.size()) / static_cast<double>(problem.num_continuous());
        } else {
            number_of_evaluations++;
        ...
        if (problem.target_reached(archive)) {
            ...
            // Note: the wonky manual short-circuiting instead of exceptions is to not mess with MO-RV-GOMEA state
            // (would crash/not free up memory)
        ...
        if (rotation_angle == 0.0) {
            ...
            if (touched_parameters_indices == NULL || black_box_evaluations ||
                ...
                if (problem_index == 9 && (learn_linkage_tree || use_univariate_FOS || static_linkage_tree)) {
                    if (i == 0)
                        sumOfEllipsoidsFunctionProblemEvaluationObjective0(
                            ind, ind->parameters, &(ind->objective_values[0]), &(ind->constraint_value));
                    else if (i == 1) {
                        ...
                        for (k = 0; k < number_of_touched_parameters; k++) {
                            if (touched_parameters_indices[k] == 0) {
                                ...
                                sumOfEllipsoidsFunctionPartialProblemEvaluationObjective1(
                                    ind, touched_parameters, 1, block_indices, touched_parameters_before,
                                    ind->objective_values[1], ind->constraint_value);
                            } else {
                                ...
                                if (cur_block != prev_block) {
                                    ...
                                    rotated_parameters_before = matrixVectorMultiplication(
                                        rotation_matrix, touched_parameters_before, block_size, block_size);
                    ...
                    rotated_parameters =
                        matrixVectorMultiplication(rotation_matrix, touched_parameters, number_of_touched_parameters,
                                                   number_of_touched_parameters);
                    rotated_parameters_before = matrixVectorMultiplication(
                        rotation_matrix, parameters_before, number_of_touched_parameters, number_of_touched_parameters);
                    installedProblemEvaluationWithoutRotation(
                        index, ind, rotated_parameters, &(ind->objective_values[i]), &(ind->constraint_value),
                        number_of_touched_parameters, touched_parameters_indices, rotated_parameters_before,
                        objective_values_before, constraint_value_before, i);
                    free(rotated_parameters_before);
    ...
    if (black_box_evaluations || touched_parameters_indices == NULL) {
        ...
        switch (index) {
            ...
            case 6:
                ...
                break;
            case 7:
                genMEDConvex2DFunctionPartialProblemEvaluation(
                    ind, number_of_touched_parameters, touched_parameters_indices, parameters_before,
                    objective_values_before, constraint_value_before, objective_index);
                break;
            case 8:
                genMEDConcave2DFunctionProblemEvaluation(parameters, objective_value_result, constraint_value_result,
                                                         objective_index);
    ...
    for (i = 0; i < default_front_size; i++) {
        ...
        for (j = 0; j < approximation_front_size; j++) {
            if (approximation_front[j]->constraint_value == 0 && to_be_removed_solution[j] == 0) {
                distance =
                    distanceEuclidean(default_front[i], approximation_front[j]->objective_values, number_of_objectives);
