...
FOS* learnLinkageTree(double** covariance_matrix) {
    ...
    if (learn_linkage_tree) {
        ...
        if (problem_index == 105 || problem_index == 106) {
            ...
            for (i = 0; i < mpm_length; i++) {
                for (j = 0; j < i; j++) {
                    if (mpm[i][0] < block_start || mpm[j][0] < block_start)
                        S_matrix[i][j] = randomRealUniform01();
                    else if ((mpm[i][0] - block_start) / block_size == (mpm[j][0] - block_start) / block_size)
                        S_matrix[i][j] = randomRealUniform01() + 1e8;
                    else
                        S_matrix[i][j] = randomRealUniform01() + 1e3;
                    S_matrix[j][i] = S_matrix[i][j];
                }
                S_matrix[i][i] = 0;
            }
    ...
    while (!done) {
                                            ...
                                            ordering has changed within the chain while merging within that chain */
        {
            ...
            new_FOS->sets[FOS_index] =
                (int*)Malloc((mpm_number_of_indices[r0] + mpm_number_of_indices[r1]) * sizeof(int));
...
void hungarianAlgorithmAddToTree(int x,
                                 int prevx,
                                 short* S,
                                 int* prev,
                                 int* slack,
